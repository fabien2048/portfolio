...
export default function Navbar() {
  ...
  // Animation séquentielle au changement de page (Home <-> Projet)
  useEffect(() => {
    const svgEl = logoSvgRef.current;
    const txtEl = logoTextRef.current;
    if (!svgEl || !txtEl) return;
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      tl.fromTo(backdropRef.current, { opacity: 0 }, { opacity: 1, duration: 0.5, ease: 'power2.out' });
      tl.fromTo(panelRef.current, { x: '100%' }, { x: '0%', duration: 0.7, ease: 'power4.out' }, '<0.1');
      tl.fromTo(
      ...
      if (menuRef.current) {
        ...
        tl.to(panelRef.current, { x: '100%', duration: 0.6, ease: 'power3.inOut' });
        tl.to(backdropRef.current, { opacity: 0, duration: 0.4, ease: 'power2.inOut' }, '<0.2');
      }
    }
  }, [isOpen, lenis]);
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 w-full z-[1000] pointer-events-none mix-blend-difference text-white">
        <div className="relative flex items-center justify-between w-full pointer-events-none px-4 py-6 md:px-6 md:py-7 lg:px-8 xl:px-12">

          {/* 1. Logo Block */}
          <div className="flex items-center pointer-events-none z-10">
            <button
              onClick={() => handleNav('/')}
              className="bg-transparent border-0 outline-none cursor-pointer hover:opacity-50 transition-opacity duration-300 pointer-events-auto grid items-center min-h-[28px] w-[128px] md:w-[204px]"
            ...
            >
              ...
              <span 
                ref={logoSvgRef} 
                className="row-start-1 col-start-1 flex items-center justify-start" 
                style={{ height: '28px' }}
              >
              ...
              <span 
                ref={logoTextRef}
                className="row-start-1 col-start-1 text-[8px] min-[375px]:text-[9px] sm:text-[10px] lg:text-[14px] font-lausanne font-medium text-white whitespace-nowrap"
                style={{ height: '100%', display: 'flex', alignItems: 'center', gap: '0.45em' }}
              >
                ...
                <span style={{ overflow: 'hidden', display: 'flex' }}>
                  <span className="logo-word block">Direction Artistique</span>
                </span>
              </span>
            </button>
          </div>
          ...
          {/* 2. Center Links */}
          <div className="hidden lg:flex items-center gap-[12px] xl:gap-[16px] text-[20px] xl:text-[22px] font-lausanne font-medium pointer-events-auto z-10 whitespace-nowrap">
            <button onClick={() => handleNav('/about')}      className={linkClass('/about')}>À propos</button>
            <button onClick={() => handleNav('/')}           className={linkClass('/')}>Projets</button>
            <button onClick={() => handleNav('/playground')} className={linkClass('/playground')}>Playground</button>
          </div>

          {/* 3. Right Side: Contact & Mobile */}
          <div className="flex items-center justify-end pointer-events-auto z-10">
            <div className="hidden lg:flex items-center">
              <a
                ...
                className={cn(
                  'relative py-1 text-[20px] xl:text-[22px] font-lausanne font-medium no-underline whitespace-nowrap',
            ...
            <div className="flex lg:hidden">
              <button
                ...
                className={cn(
                  'bg-transparent border-0 outline-none cursor-pointer h-[44px] flex items-center gap-2 px-2 text-white transition-all',
                  isOpen ? 'opacity-0 invisible' : 'opacity-100 visible'
                )}
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M20.2766 4.01803L7.29049 17.0042C7.03561 17.1991 6.71556 17.3869 6.49764 17.511C6.45143 17.5347 6.41899 17.5636 6.40103 17.5986C6.38529 17.6288 6.37354 17.3869 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986C6.38529 17.6288 6.37354 17.6785 6.40103 17.5986Z" fill="currentColor"/>
                </svg>
                <span className="text-[16px] font-lausanne font-medium">Menu</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <div
        ref={menuRef}
        className="fixed inset-0 pointer-events-auto invisible flex justify-end lg:hidden"
        style={{ zIndex: 1001 }}
      >
        <div ref={backdropRef} className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setIsOpen(false)} />
        <div ref={panelRef} className="relative w-[75vw] sm:w-[60vw] h-full bg-[#0B0B0B] text-[#FDE9CE] flex flex-col" style={{ transform: 'translateX(100%)' }}>
          <div className="flex justify-end pt-6 pr-6 md:pt-8 md:pr-8">
            <button onClick={() => setIsOpen(false)} className="bg-[#FDE9CE] text-[#0B0B0B] rounded-[20px] px-4 py-2 text-[12px] md:text-[14px] font-bold uppercase flex items-center gap-2 cursor-pointer border-0">
              <span className="w-1.5 h-1.5 bg-[#0B0B0B] rounded-full" />
              Fermer
            </button>
          </div>
            ...
            <nav className="flex flex-col gap-4">
              {([
                ...
                <div key={to} ref={el => { linksRef.current[i] = el; }}>
                  <button onClick={() => handleNav(to)} className={cn('block bg-transparent border-0 cursor-pointer text-left text-[10vw] sm:text-[8vw] font-lausanne font-bold uppercase transition-opacity', location.pathname === to ? 'text-[#FDE9CE]' : 'text-[#FDE9CE] hover:opacity-70')}>
                    {label}
                  </button>
                </div>
              ))}
            </nav>
            <div ref={contactRef} className="mt-16 flex flex-col gap-1">
              <span className="text-[11px] font-lausanne font-bold text-[#FDE9CE]/50 uppercase tracking-widest">General Enquires:</span>
              <a href="mailto:f.bouadi@gmail.com" className="text-[16px] font-lausanne font-bold text-[#FDE9CE] uppercase no-underline">f.bouadi@gmail.com</a>
            </div>
          </div>
        </div>
      </div>
